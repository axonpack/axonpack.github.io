...
| `supportedTypes` | `StorageValueType[]`                       | What the store can actually hold: `'string'`, `'number'`, `'boolean'`, `'buffer'`. Defaults to all four. |
...
The list of types the store can really hold, out of `'string'`, `'number'`, `'boolean'` and
`'buffer'`; it defaults to all four. `asyncStorageAdapter` and
