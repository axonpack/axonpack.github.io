...
the way it left: as text. The sheet takes a paste (or reads the clipboard for you), then says what the
file would do before anything is written — how many keys are new, how many would be overwritten, how
many already hold exactly that value, and how many are skipped, with the reason: hidden by the
blacklist, a type this store does not hold, or no value to write. A file exported from a different store
