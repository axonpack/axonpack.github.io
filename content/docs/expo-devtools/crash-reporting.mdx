...
The errors that end a session, or nearly do, turned into a report you can read on the device — and the
one subsystem here meant to survive into a release build.
...
<Callout type="info" title="Two sheets, and the wrong one in release is a real problem">
  `popupDetail` defaults to `'auto'`: the full developer sheet — tabs, a stack tree, raw JSON, this
  package's branding — when the devtools are enabled, and a compact notice — what broke, when, and
  Share / Copy / Dismiss — when they are not. Set it explicitly for an internal build that ships the
  crash sheet but not the panel and still wants the stack on screen.
</Callout>
