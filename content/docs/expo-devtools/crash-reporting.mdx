...
- **A fatal JS error still ends the app.** The `ErrorUtils` handler is wrapped, not replaced: the
  record is taken, then the error goes straight on to whatever was installed before it. React Native
  then does what it always did. In a release build that means reporting it to the native side, which
  is what ends the process, so the crash comes back at the next launch as the native exception it
  became. In development the same handler draws the red box instead, and the app carries on. Every
  other reporter you have installed, Sentry or Crashlytics or your own, sees the error too.
