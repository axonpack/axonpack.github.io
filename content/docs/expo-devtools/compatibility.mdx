...
| ------------------------- | -------- | ------------ | ----- |
| <VersionRange slug="expo-devtools" /> | 57 | 0.86 | 19.2 |
...
The [React Native DevTools tab](/docs/expo-devtools/react-native-devtools) comes from
`@axonpack/react-native-devtools-tab`, which this package depends on. It needs React 19.2 or later,
and a React Native with React Native DevTools, the debugger `j` opens. It is built and tested against
the same row.
...
All three are peer dependencies, so they come from your app rather than from this package:
